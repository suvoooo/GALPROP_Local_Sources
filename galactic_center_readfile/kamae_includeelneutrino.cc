...
//**.****!****.****!****.****!****.****!****.****!****.****!****.****!****.****!
//***************                 SB 2020/02/13          **********************!
// Implementation of parametrization of proton ----> charged pions ----> neutrino production cross-section 
// the formulas are diffrent for electron and muon neutrino so they will be implemented separately 
//***************                 SB 2020/02/13          **********************! 
...
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
//		calculate non-diff parameters for electron neutrino
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
valarray<double> kamae_gamma_param_nd(double Tp) {

  valarray<double> param_a(0., 9);

  double y, z, z1, z2, z3, z4, z5, z6, z7;
  int i;
  ...
  if ((Tp > 0.487) && (Tp < 512000.1)) {
    ...
    param_a[0] = 0.0074087 + z*(2.9161 + z*(0.99061 + z*(-0.28694 + 0.038799*z)));
    param_a[1] = -3.2480e-5 + 7.1944e-5 * exp(-0.21814 * (z1)) + (2.0467e-5 / (z2) ) + (5.6954e-6 * y) - (3.4105e-7 * (y * y)) ;
    param_a[2] = -230.50 + y*(58.802 + y*(-9.9393 + y*(1.2473 - 0.26322*y))) ;
    param_a[3] = 0.45064 + y*(0.56930 + y*(0.012428 - 0.0070889*y)) ; 
    param_a[4] = -0.011883 + (z3 * (1.7992 + z3*(3.5264 + z3*(-1.7478 + z3*(0.32077 - 0.017667) ) ) ) );
    param_a[5] = -1.6238e-7 + 1.8116e-6 * exp(-0.30111*z1) + (9.6112e-5/(z4*z4) );
    param_a[6] = -261.30 - (43.351 * log10(0.35298 * z1)) + ( 70.925/(z5 * z5) ) ; 
    param_a[7] = 184.45 - ( 1473.6/z6 ) - (4.0536 * (y*y) ) ; 
    param_a[8] = -0.24019 + y*(0.38504 + y*(0.0096869 - 0.0015046*y));     
...
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|*****.*****|
//		calculate diff. diss. parameters for gamma-rays (electron neutrino)
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|*****.*****|
valarray<double> kamae_gamma_param_diff(double Tp) {

  valarray<double> param_b(0., 8);

  double y, z1, z2, z3, z4, z5, z6, z7, z8, z9, z10;
  double factor, factor1, factor2, factor3, factor4, factor5 ;  
  int i;
  ...
  if ((Tp > 1.94) && (Tp < 512000.1)) {
    ...
    if (Tp > 11.51) {   
      param_b[0] = 53.809 * tanh(- 0.41421 * (z1)) - (6.7538 * (z2 * z2)) + (0.0088080 * (z3 * z3 * z3 * z3));
      factor = 1.0 + (4.4440 * z4); 
      factor1 =  1.3651 * ((z4 / factor) * (z4 / factor)) ; 
      param_b[1] = -50.211 + (55.131 * exp( factor1)); 
      param_b[2] = -17.231 + (0.041100 * tanh(7.9638 * z5)) - (0.055449 * y) + (2.5886e-4 * (z6 * z6)) ;
      factor2 = 1.0 + (5.5969 *(z7) );
      factor3 = -1.4412 * ((z7 / factor2) *  (z7 / factor2));   
      param_b[3] = 12.335 - 12.893 * exp(factor3);
    }
    ...
    factor4 = ( (4.6121 * (z1 * z1) ) + z1 ) * ( (4.6121 * (z1 * z1) ) + z1 ) ;
    factor5 = exp( -5.4007 * (factor4)) ;  
    param_b[4] = 1.3558  + (0.46601 * (z8)) + (0.052978 * (z1 * z1)) + 0.79575 * factor5;
    param_b[5] = 1.8756 - (0.42169 * tanh(1.61 * (z5)) ) - (0.051026 * z9) ; 
    param_b[6] = 1.5016 + y*(1.0118 + y*(-0.072787 + y*(-0.0038858 + (y*0.0093650) )));
    param_b[7] = 4.9735 + (5.5674 * tanh(-0.36249 * z1) ) - (0.20660 * (z10) ) ;       
  }  
...
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
//		calculate delta(1232) parameters for electron-neutrino
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
valarray<double> kamae_gamma_param_delta(double Tp) {

  valarray<double> param_c(0., 5);

  double y, factor, z1, z2, factor1;
  int i;
  ...
  if ((Tp < 0.488) || (Tp > 1.95))
    ...
    z1 = y + 3.1282 ;   
    z2 = y + 2.1;
    factor = ( z1 /  (1. + 0.48240 * (z1)) ) * ( z1 /  (1. + 0.48240 * (z1)) );
    factor1 = 71.339 * (factor) ; 
    param_c[0] = (2.8290 * exp(-factor1) ) - 9.6339 - (15.773/y) + (0.52413 * y * y) ;
    param_c[1] = -24.571 - (15.831*y) - (2.120 * y * y); 
    param_c[2] = -5.953 - (6.4695 * tanh(4.7225 * z2)) + (0.5003*y) ; 
    param_c[3] = 0.26022 + (0.24545 * y); 
    param_c[4] = 0.076498 + (0.061678 * y) + (0.0040028 * y * y); 
...
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
//		calculate res(1600) parameters for electron neutrino
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
valarray<double> kamae_gamma_param_res(double Tp) {

  valarray<double> param_d(0., 5);

  double y, z1, z2, factor, factor1;
  int i;
  ...
  } else
  {
    z1 = y + 2.9509 ; 
    z2 = y + 2.1;
    factor =  ( z1/(1.0 + (1.4101 * z1)) ) * ( z1/(1.0 + (1.4101 * z1)) );
    factor1 = -57.260 * (factor);

    param_d[0] = (1.7951 * (exp(factor1)) ) - 0.58604 - (0.23868 * y) ; 
    param_d[1] = -2.6395 - (1.5105 * y) + (0.22174 * y*y); 
    param_d[2] = -7.0512 - (7.1970 * tanh(31.074 * z2)) + (0.39007 * y); 
    param_d[3] = -1.4271 - (1.0399 * y) - (0.24179 * y * y); 
    param_d[4] = 0.74875 + (y * (0.63616 + y*(0.17396 + (0.017636*y)) ));  
...
double kamae_nd(double E, double Tp, valarray<double> param_a, int particle) {
  ...
  // check table 2 for electron neutrino params.. 
  double L_MAX[3] = {0.98, 0.96, 0.94}; // 0.98 for electron neutrino
  // double W_NDL[3] = {15.0, 20.0, 15.0};
  double W_NDL[3] = {15., 20., 15.}; // 15 for electron neutrino 
  // double W_NDH[3] = {44.0, 45.0, 47.0};
  double W_NDH[3] = {42.0, 45.0, 47.0}; // 42 for electron neutrino 
  
  // init some variables, given in table 2
  Lmin = -2.6; // this value is same for all secondaries SB 2020/02/18 
  ...
  case 0: // gamma-rays ------> electron neutrino
  // these values are also updated for electron neutrino
  // Tp limit is set for 7.81 GeV // SB 2020/02/18  
    if (Tp <= 7.81) {

      // pow1 = (y + 3.25)/(1.0 + 8.08*(y + 3.25));
      z1 = (y + 3.26);
      pow1 = (z1/(1 + (6.56*z1) ) ) * (z1/(1 + (6.56*z1) ) );
      r_factor = 0.329*exp(-247*pow1) - (0.957*y) - (0.229 * y * y);
    }
    else
      r_factor = 1.0;
    break;
