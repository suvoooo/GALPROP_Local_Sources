...
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
//		calculate non-diff parameters for muon neutrino
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
valarray<double> kamae_gamma_param_nd(double Tp) {
  ...
  if ((Tp > 0.487) && (Tp < 512000.1)) {
    ...
    z = y + 3.3;
    param_a[0] = z*(-0.63611 + z*(9.9015 + z*(-4.5897 + z*(0.91778 - 0.060724*y))));
    param_a[1] = 6.87e-6 + y*(-2.8245e-6 + y*(7.6032e-7 + y*(-3.2953e-7 + 7.492e-8*y)));
    param_a[2] = -240.46 + y*(58.405 + y*(-9.8556 + y*(3.1401 - 0.88932*y)));
    param_a[3] = 0.49935 + y*(0.60919 + y*(0.0024963 - 0.0099910*y));
    z1 = y + 3.32;
    param_a[4] = z1*(2.5094 + z1*(4.1350 + z1*(-0.89534 + z1*(-0.0027577 + 0.014511*z1))));
    z2 = y + 4.7136;
    param_a[5] = 8.2046e-7 + 1.4085e-6*log10(0.016793*(y + 4.3)) + 0.00013340/(z2*z2) ;
    param_a[6] = -267.55 - 0.21018*log10(0.35217*(y + 3.9)) + (y*(69.586 - 9.9930*y));
    param_a[7] = 2742.8 + (222.01 * log10(0.35217 * (y+3.9)) ) - (4772.5/(y+19.773)) - (6.1001 * y * y);
    param_a[8] = -0.11857 + y*(0.39072 + y*(-0.0378183 + y*(0.0022265 + 0.0046931*y)));
  } //else {
...
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
//		calculate diff. diss. parameters for muon neutrino
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
valarray<double> kamae_gamma_param_diff(double Tp) {

  valarray<double> param_b(0., 8);

  double y, z1, z2, z3, z4, z5, wop;
  double factor, factor1, factor2, z6;
  int i;
  ...
  if ((Tp > 1.94) && (Tp < 512000.1)) {
    ...
    if (Tp > 5.51) {
      ...
      z1 = y + 0.44754;
      z2 = y + 9.9165;
      param_b[0] = 64.682*tanh(-0.34313*(y + 2.2)) - (5.5955*z1*z1) + (0.0050117*z2*z2*z2*z2);
      z3 = y + 2.3066;
      factor = (z3/(1.0 + (41.612 * z3) ) ) * (z3/(1.0 + (41.612 * z3) ) );
      param_b[1] = -7.6016 + (3.0427e4 * exp(-1.013e4 * factor) );
      param_b[2] = -1.4978 - 0.58163*tanh(-0.36488*(y + 1.9)) + (0.031825 * (y + 2.8097) ) + (0.022796*(y - 1.8861) * (y-1.8861));
      z4 = y + 3.8835;
      wop = (z4/(1.0 + (0.53*z4) ) ) * (z4/(1.0 + (0.53*z4) ) );
      param_b[3] = -0.0061483 - (65.799 * exp(-4.82399 * wop) );
    ...
    z5 = y + 2.95;
    z6 = y + 2.2;
    factor1 = 10.592 * (z6 - 0.19149*(z6*z6)) * (z6 - 0.19149*(z6*z6));
    param_b[4] = 2.8009 + (0.35341 * z5 * z5) - (0.0039779 * z5*z5*z5*z5) + (1.3012 * exp(-factor1) );
    param_b[5] = 1.8016 - (0.69847 * tanh(2.8627 * (y+1.9)) ) - (0.015722 * (y - 45.4)) ;
    param_b[6] = 1.4617 + (y*(1.0167 + y*(-0.078617 + y*(-0.0038336 + (0.010141*y)))) ) ;
    // z1 = y + 1.8441;
    param_b[7] = 3.5599 + (4.0041*tanh(-0.41889 * (y + 2.1)) ) - (0.18182 * (y - 2.4209) );
  } //else {
...
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
//		calculate delta(1232) parameters for muon neutrino
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
valarray<double> kamae_gamma_param_delta(double Tp) {

  valarray<double> param_c(0., 5);

  double y, z1, wop, factor;
  int i;
  ...
  if ((Tp < 0.488) || (Tp > 1.95))
    ...
    z1 = y + 3.1278;
    wop = (z1/(1.0 - (0.19497 * z1)) ) * (z1/(1.0 - (0.19497 * z1)) );
    factor = -60.914 * wop ; 
    param_c[0] = (3.6052*exp(factor) ) - 0.92514 + (2.1315/y) + (0.23548 * y * y);
    param_c[1] = 95.310 + y*(70.497 + 13.636*y);
    param_c[2] = -6.2158 - 6.2939*tanh(21.592*(y + 2.1)) + 0.37440*y;
    param_c[3] = 2.7485 + 1.1692*y;
    param_c[4] = -2.7568 + y*(-1.8461 - 0.31376*y);
...
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
//		calculate res(1600) parameters for muon neutrino
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
valarray<double> kamae_gamma_param_res(double Tp) {

  valarray<double> param_d(0., 5);

  double y, wop, factor, z1;
  int i;
  ...
  if ((Tp < 0.69) || (Tp > 2.76)) {
    ...
    z1 = y + 2.9509; 
    wop = (z1/(1.0 + (1.315*z1 )) ) * (z1/(1.0 + (1.315*z1 ) ) );
    factor = -58.488 * wop ;  
    param_d[0] = 2.5489 * (exp(factor)) - 0.83039 - (0.34412*y) ;
    param_d[1] = 88.173 + (65.148 * y) + (12.585 * y * y);
    param_d[2] = -7.0962 - (7.1690*tanh(30.890*(y + 2.1)) ) + ( 0.38032*y );
    param_d[3] = -4.1440 + y*(-3.2717 - 0.70537*y);
    param_d[4] = 2.2624 + y*(1.1806 + y*(-0.0043450 - 0.043020*y));    
  }
  return param_d;
...
double kamae_nd(double E, double Tp, valarray<double> param_a, int particle) {
  ...
  // Table 2 of Kamae et al. // check Table 2 for muon neutrino values
  // double L_MAX[3] = {0.96, 0.96, 0.94};
  double L_MAX[3] = {0.94, 0.96, 0.94}; // muon neutrino factor 0.94 * log(Tp)
  // double W_NDL[3] = {15.0, 20.0, 15.0};
  double W_NDL[3] = {20.0, 20.0, 15.0}; // muon neutrino factor 20.
  // double W_NDH[3] = {44.0, 45.0, 47.0};
  double W_NDH[3] = {45.0, 45.0, 47.0}; // muon neutrino factor 45
  
  // init some variables, given in table 2
  Lmin = -2.6; //same for all secondaries SB 2020/02/18
  ...
  case 0: // gamma-rays // now written for muon neutrino
    // limit for Tp is set to 15.6 GeV; SB 2020/02/18  
    if (Tp <= 15.95) {

      z1 = y + 3.25 ; 
      pow1 = (z1/(1.0 + 8.38*(z1)) ) * (z1/(1.0 + 8.38*(z1)) );
      r_factor = 2.23*exp(-93.4*pow1) - (0.376*y) - (0.121 * y *y);
    }
    else
      r_factor = 1.0;
    break;
