  ...
  */
  ...
  for (i = 0; i < n_species; ++i)
  {
    ...
    if (gcr[i].A > particle.A && !particle.K_electron)
    {
      ...
      if (galdef.verbose == -601 || galdef.verbose == 10)
      { // selectable debug
      ...
      if (gcr[i].A == particle.A && gcr[i].Z != particle.Z)
                    ...
                    &t_half))
        {
          ...
          if (t_half > 0. && t_half / year2sec < galdef.t_half_limit)
          {
            ...
            if (galdef.verbose == -602 || galdef.verbose >= 1)
            { // selectable debug                  //AWS20010828
    ...
    if (galdef.verbose == -603 || galdef.verbose == 10)
    { // selectable debug
              ...
              for (int iz = 0; iz < gcr[0].n_zgrid; iz++)
                for (int ip = 0; ip < gcr[0].n_pgrid; ip++)
                  decay_source.d2[ir][iz].s[ip] =
                      branching_ratio * gcr[i].cr_density.d2[ir][iz].s[ip] / (gcr[i].gamma[ip] * t_half / log(2.0));
                ...
                for (int iz = 0; iz < gcr[0].n_zgrid; iz++)
                  for (int ip = 0; ip < gcr[0].n_pgrid; ip++)
                    decay_source.d3[ix][iy][iz].s[ip] =
                        branching_ratio * gcr[i].cr_density.d3[ix][iy][iz].s[ip] / (gcr[i].gamma[ip] * t_half / log(2.0));
...
//**"****!****"****!****"****!****"****!****"****!****"****!****"****!****"****|
int Galprop::gen_TD_secondary_source(Particle &particle)
{ //YO/20140221
