...
int Galprop::create_gcr() //AWS20050816
{
   ...
   if (!galdef.secondary_antiprotons)
      galdef.tertiary_antiprotons = 0; // IMOS20000802

   n_species = 0;
   if (galdef.secondary_positrons)
      n_species++;
   if (galdef.knock_on_electrons)
      n_species++; //IMOS20060504
   ...
   if (galdef.primary_TD_positron)
      n_species++; //YO/20140220 -> YO/20140408 changed
   if (galdef.primary_TD_electron)
      n_species++; //YO/20140408
   ...
   for (Z = 1; Z <= galdef.max_Z; Z++)
   {
      ...
      {
         if (!nucdata(galdef_network_par, Z, A, 0, Z, A, &Z1, &A1, &t_half[0]))
            continue;
         if (!nucdata(galdef_network_par, Z, A, 1, Z, A, &Z1, &A1, &t_half[1]))
            continue;
         for (K_electron = 0; K_electron <= galdef.K_capture; K_electron++)
         {
            if (t_half[K_electron] > 0. && t_half[K_electron] / year2sec < galdef.t_half_limit)
               continue;
            if (K_electron)
   ...
   if (galdef.verbose >= 1)
      cout << endl
           << "Number of species to create: n_species= " << n_species << endl
           << endl;
   if (!n_species)
   {
   ...
   if (galdef.DM_positrons)
   {
      ...
      if (galdef.verbose >= 1)
         gcr[i].print();
      if (galdef.verbose >= 1)
         cout << "============== completed creation of " << gcr[i].name << endl
              << endl;
      i++;
   }
   ...
   for (Z = 1; Z <= galdef.max_Z; Z++)
   {
      ...
      for (A = 2 * Z - 2; A < 2.5 * Z + 4.2; A++)
      {
         ...
         for (K_electron = galdef.K_capture; K_electron >= 0; K_electron--) // AWS20010731
         {
            ...
            for (j = 1; j < 31; j++)
               if (Z == j)
                  sprintf(name, "%s_%d", element[j - 1], A);

            gcr[i].primary_abundance = galdef.isotopic_abundance[Z][A];
            if (K_electron > 0)
               gcr[i].primary_abundance = 0.0; // no prim. K-capture nuclei AWS20010731

            if (K_electron == 1)
               strcat(name, "*"); // AWS20010731
