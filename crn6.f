...
C     SELECTION OF ENERGY IENER(LP,JS)<T<IENER(LP+1,JS)
  ...
  90  LP=L-1
      ...
      NPAR=1
C     NEE=3
C     We do linear interpolation in energy so NEE=2 should be used
C     NEE=3 actually results in reading out of bounds, see loop 50
      NEE=2
...
c         Mo,  Cd,    Sn,    Ba,     W,    Pb,     U,     Cf          
     ...
     *95.94, 112.40, 118.69, 137.34,183.85,207.19,238.03,250.0/
      ...
      SUBROUTINE SIGTAP2 (ITYPE,FILENAME,FNLENGTH)
      IMPLICIT REAL*8 (A-H,O-Z)
...
C     L- VS. ENERGY
C
      OPEN(2,FILE=FILENAME,STATUS='OLD',FORM='FORMATTED')
c'barpol.dat',STATUS='OLD',FORM='FORMATTED')  
