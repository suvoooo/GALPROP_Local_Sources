    ...
    for (int ir = 0; ir < gcr[0].n_rgrid; ir++)
    {
      for (int iz = 0; iz < gcr[0].n_zgrid; iz++)
      {
        for (int ip = 0; ip < particle.n_pgrid; ip++)
        {
          ...
          if (abs(galdef.DM_int0) == 99 && particle.A == 0)
          {
            if (strcmp(particle.name, "DM_electrons") == 0) //numerical  solution "DM_electrons"
              particle.secondary_source_function.d2[ir][iz].s[ip] =
                  (galdef.DM_double6 <= fabs(galaxy.z[iz])) ? 0. : C / 4. / Pi * pow(particle.Ekin[ip] / galdef.DM_double8, -galdef.DM_double9);
          ...
          {
            if (DME0 < particle.Ekin[ip] || DME0 / DMwidth > particle.Ekin[ip])
              continue;
          ...
          if (particle.Etot[ip] * 1.e-3 <= DMmass)
            particle.secondary_source_function.d2[ir][iz].s[ip] += DMcs_v *
                                                                   pow(DM_profile_av(galaxy.r[ir], galaxy.z[iz], galaxy.dr, galaxy.dz, dzz) / DMmass, 2) * C / 4. / Pi * DMbranching * exp(-pow((DMmass - particle.Etot[ip] * 1.e-3) / DMwidth, 2)) / DMmass * 1.e-3;
        } // ip
      }   //  iz
    }     //  ir
        ...
        for (int iz = 0; iz < gcr[0].n_zgrid; iz++)
        {
          for (int ip = 0; ip < particle.n_pgrid; ip++)
          {
            if (galdef.DM_int1 == 9) // Green's function to work with DarkSUSY IMOS20060901
            {
              if (DME0 < particle.Ekin[ip] || DME0 / DMwidth > particle.Ekin[ip])
                continue;
              particle.secondary_source_function.d3[ix][iy][iz].s[ip] += pow(DM_profile_av(galaxy.r[ix], galaxy.r[iy], galaxy.z[iz], galaxy.dx, galaxy.dy, galaxy.dz, dzz), 2) * DMsecondary_spectrum * DMbranching / 4. / Pi * C;
              continue;
            }
            if (particle.Etot[ip] * 1.e-3 <= DMmass)
              particle.secondary_source_function.d3[ix][iy][iz].s[ip] += DMcs_v *
                                                                         pow(DM_profile_av(galaxy.x[ix], galaxy.y[ix], galaxy.z[iz], galaxy.dx, galaxy.dy, galaxy.dz, dzz) / DMmass, 2) * C / 4. / Pi * DMbranching * exp(-pow((DMmass - particle.Etot[ip] * 1.e-3) / DMwidth, 2)) / DMmass * 1.e-3;
          } //ip
        }   //  iz
      }     //  iy
    }       //  ix
      ...
      for (int iz = 0; iz < gcr[0].n_zgrid; iz++)
      {
        for (int iEgamma = 0; iEgamma < galaxy.n_E_gammagrid; iEgamma++)
        {
          ...
          galaxy.DM_emiss.d2[ir][iz].s[iEgamma] = DMcs_v * DMbranching / (4. * Pi) // sr^-1 IMOS20060420
                                                  * pow(DM_profile_av(galaxy.r[ir], galaxy.z[iz], galaxy.dr, galaxy.dz, dzz) / DMmass, 2) / galaxy.E_gamma[iEgamma];
        }
      }
    }
  }
        ...
        for (int iz = 0; iz < gcr[0].n_zgrid; iz++)
        {
          for (int iEgamma = 0; iEgamma < galaxy.n_E_gammagrid; iEgamma++)
          {
            ...
            galaxy.DM_emiss.d3[ix][iy][iz].s[iEgamma] = DMcs_v * DMbranching / (4. * Pi) // sr^-1 IMOS20060420
                                                        * pow(DM_profile_av(galaxy.x[ix], galaxy.y[ix], galaxy.z[iz], galaxy.dx, galaxy.dy, galaxy.dz, dzz) / DMmass, 2) / galaxy.E_gamma[iEgamma];
          }
        }
      }
    }
...
double Galprop::DM_profile(double Xkpc, double Ykpc, double Zkpc)
{
  ...
  case 2: //Evans profile
    return (rho0 * pow(pow(Rc, 2) + pow(Rsun, 2), 2) / (3. * pow(Rc, 2) + pow(Rsun, 2)) * (3. * pow(Rc, 2) + pow(R, 2)) / pow(pow(Rc, 2) + pow(R, 2), 2));
      ...
      for (int ir = 0; ir < gcr[0].n_rgrid; ir++)
      {
        for (int iz = 0; iz < gcr[0].n_zgrid; iz++)
        {
          for (int ip = 0; ip < particle.n_pgrid; ip++)
          {
            ...
            if ((particle.r[ir] == -20 && particle.z[iz] == -4) && abs(particle.p[ip] - mod_DM_mass) < 0.01)
              cerr << particle.secondary_source_function.d2[ir][iz].s[ip] << endl;
          } // ip
        }   //  iz
      }     //  ir
    }       //  particle.n_spatial_dimensions==2
...
double Galprop::func_DM_Annihilation(double E_, double mod_DM_mass_)
{
  ...
  {
  case 1: //mono
    if (abs(E - mod_DM_mass) < 0.001)
      source = galdef.power_law_1;
    else
      source = 0;
    break;
  case 2: //flat
    if (E <= mod_DM_mass)
      source = 1 / (mod_DM_mass - 0);
    else
      source = 0;
    break;
  case 3: //double-peak
    if (E <= mod_DM_mass)
      source = 3 / pow(mod_DM_mass, 3) * (pow(E - mod_DM_mass / 2, 2) + pow(mod_DM_mass, 2) / 4);
    else
      source = 0;
    break;
  }
...
int Galprop::SNR_burstlike(Particle &particle) //YO/20141010 -> 20141117
{
  ...
  double E_cut = galdef.energy_cutoff;
  double index;
  double tot_en = galdef.total_energy; //sb20191025
  ...
  if (strcmp(particle.name, "primary_TD_electron") == 0)
  {
        ...
        particle.m_source_age /*galdef.source_age*/ >= (particle.m_final_time - particle.m_step_total_time))
    {
      ...
      if (galdef.check_num == 325)
      {
        cerr << " SNR model = " << galdef.TD_SNR_model << endl;
        // cerr << " total time - propagating time= " << particle.m_final_time - particle.m_step_total_time
        cerr << " source_age = " << particle.m_source_age /*galdef.source_age*/ << endl;
        cerr << " => USE CR source" << endl;
        // cerr<<
      }
          ...
          for (int iy = 0; iy < gcr[0].n_ygrid; iy++)
          {
            for (int iz = 0; iz < gcr[0].n_zgrid; iz++)
            {
              for (int ip = 0; ip < particle.n_pgrid; ip++)
              {
                ...
                {
                  if (particle.p[ip] < galdef.power_law_br)
                    index = galdef.power_law_1;
                  if (particle.p[ip] >= galdef.power_law_br)
                    index = galdef.power_law_2;
                  ...
                  // particle.secondary_source_function.d3[ix][iy][iz].s[ip] = func_SNR_burstlike(particle.p[ip], E_cut, index) * C / 4 / Pi;
                  particle.secondary_source_function.d3[ix][iy][iz].s[ip] = 1.0 * func_SNR_burstlike(particle.p[ip], E_cut, index) * C / 4 / Pi; //sb 20191025
                  if (galdef.check_num == 1117)
                  {
                    cerr << " source_coord (" << particle.x[ix] << "," << particle.y[iy] << "," << particle.z[iz] << ")";
                    // cerr << ", norm_fac = " << particle.m_nor_fac;
                    cerr << " : func_SNR_burst = " << func_SNR_burstlike(particle.p[ip], E_cut, index) << endl;
                    cerr << " total time - propagating time= " << particle.m_final_time - particle.m_step_total_time << endl; //sb20191019
                  }
                }
                else
                  particle.secondary_source_function.d3[ix][iy][iz].s[ip] = 0;
...
int Galprop::nor_SNR_burstlike(Particle &particle)
{
  ...
  x = E_min;
  if (x >= galdef.power_law_br)
    index = galdef.power_law_2 - 1;
...
int Galprop::SNR_exp_decay(Particle &particle) //YO/20141010 -> 20141202
{
          ...
          for (int iy = 0; iy < gcr[0].n_ygrid; iy++)
          {
            for (int iz = 0; iz < gcr[0].n_zgrid; iz++)
            {
              for (int ip = 0; ip < particle.n_pgrid; ip++)
              {
                ...
                {
                  particle.secondary_source_function.d3[ix][iy][iz].s[ip] = particle.m_nor_fac * \
                  func_SNR_exp_decay1(particle.p[ip], index) * \
                  func_SNR_exp_decay2((galdef.source_age - (particle.m_final_time - particle.m_step_total_time))) * C / 4 / Pi;
                  if (galdef.check_num == 1117)
                  {
                    cerr << " source coord (" << particle.x[ix] << "," << particle.y[iy] << "," << particle.z[iz] << ")";
                    cerr << ", norm fac = " << particle.m_nor_fac;
                    cerr << " : second_source_func = " << particle.secondary_source_function.d3[ix][iy][iz].s[ip] << endl;
                  }
                }
                else
                  particle.secondary_source_function.d3[ix][iy][iz].s[ip] = 0;
  ...
  if (strcmp(particle.name, "primary_TD_electron") == 0)
  {
    ...
    if ((particle.m_final_time - particle.m_step_total_time) <= galdef.source_age)
    // if ((particle.m_final_time - particle.m_step_total_time) >= galdef.source_age) //issue1 // checked and results irrelevant
    {
      ...
      if (galaxy.n_spatial_dimensions == 3)
      {
        for (int ix = 0; ix < gcr[0].n_xgrid; ix++)
        {
		  ...
		  source*C/4/Pi*exp(-((galdef.source_age-((particle.m_final_time-particle.m_step_total_time)))*log(4))/tau_0);*/
                ...
                {
                  // particle.secondary_source_function.d3[ix][iy][iz].s[ip] = particle.m_nor_fac * 
                  particle.secondary_source_function.d3[ix][iy][iz].s[ip] = 1. * \
                  func_SNR_exp_decay1(particle.p[ip], index) * \
                  func_SNR_exp_decay2((- galdef.source_age + (particle.m_final_time - particle.m_step_total_time))) * C / 4 / Pi; //  changed SB20191105
                  // func_SNR_exp_decay2((galdef.source_age - (particle.m_final_time - particle.m_step_total_time))) * C / 4 / Pi; //
                  if (galdef.check_num == 1117)
                  {
...
//Get normalization factor with Simpson method
int Galprop::nor_SNR_exp_decay(Particle &particle)
{ //YO/20141203 未完
  ...
  x1 = E_min;
  integral1 = func_SNR_exp_decay1(x1, index); // whether E_cut1 is necessary or not SB20191024/

  for (double i = 1; i < div_num1; i = i + 1.)
  {
    dy1 = 4 * func_SNR_exp_decay1((x1 + dx1), index) + 2 * func_SNR_exp_decay1((x1 + 2 * dx1), index); // E_cut1 //
    integral1 += dy1;
    x1 += 2 * dx1;
  }
...
double Galprop::func_SNR_exp_decay1(double E, double a) // should E_cut1 be a variable or not. SB20192410// 
{ //YO/20141203
  double result1;
  double E_cut1 = galdef.energy_cutoff;
  // result1 = pow(E, -a);
  result1 = pow(E, -a) * exp((-E / E_cut1)); //changed by saptashwa
  return result1;
}
double Galprop::func_SNR_exp_decay2(double t)
{ //YO/20141204
  double result2;
  // double tau_0 = 1.e5;
  // double tau_0 = 1.e7;
...
double Galprop::flux_escape1(Particle &particle, int ip, double E_esc, double tau_0, double source)
{ //YO/20141024
  double source1;
...
double Galprop::flux_escape2(Particle &particle, int ip, double E_esc, double a, double t_sedov, double source)
{ //YO/20141024
  double source2 = 0;
  double N_conf = 0;
  double t, t_cre;
