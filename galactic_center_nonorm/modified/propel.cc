				...
				for (iz = 0; iz < particle.n_zgrid; ++iz)
				{
					...
					for (ip = 1; ip < particle.n_pgrid - 1; ++ip)
					{
						...
						alpha1_p.d2[ir][iz].s[ip] +=
							(-(particle.Dpp.d2[ir][iz].s[ip] - particle.Dpp.d2[ir][iz].s[ip - 1]) / (particle.p[ip] - particle.p[ip - 1]) + 2. * particle.Dpp.d2[ir][iz].s[ip] / (particle.p[ip + 1] - particle.p[ip - 1]) + 2. * particle.Dpp.d2[ir][iz].s[ip - 1] / particle.p[ip - 1]) / (particle.p[ip] - particle.p[ip - 1]);

						alpha2_p.d2[ir][iz].s[ip] +=
							-(particle.Dpp.d2[ir][iz].s[ip] - particle.Dpp.d2[ir][iz].s[ip - 1]) / pow(particle.p[ip] - particle.p[ip - 1], 2.) + 2. * particle.Dpp.d2[ir][iz].s[ip] / (particle.p[ip + 1] - particle.p[ip - 1]) * (1. / (particle.p[ip + 1] - particle.p[ip]) + 1. / (particle.p[ip] - particle.p[ip - 1])) + 2. * particle.Dpp.d2[ir][iz].s[ip] / (particle.p[ip] - particle.p[ip - 1]) / particle.p[ip];

						alpha3_p.d2[ir][iz].s[ip] +=
							2. * particle.Dpp.d2[ir][iz].s[ip] / (particle.p[ip + 1] - particle.p[ip - 1]) / (particle.p[ip + 1] - particle.p[ip]);
					...
					for (ir = 0; ir < particle.n_rgrid; ++ir)
					{
						...
						for (ip = 0; ip < particle.n_pgrid; ++ip)
						{
							...
							for (iz = 0; iz < particle.n_zgrid; ++iz)
							{
								...
								if (iz > 0)
									Nz0[iz] += 0.5 * alpha1_z.d2[ir][iz].s[ip] * particle.cr_density.d2[ir][iz - 1].s[ip];

								if (iz < particle.n_zgrid - 1)
									Nz0[iz] += 0.5 * alpha3_z.d2[ir][iz].s[ip] * particle.cr_density.d2[ir][iz + 1].s[ip];
							}
					...
					for (ir = 0; ir < particle.n_rgrid; ++ir)
					{
						...
						for (iz = 0; iz < particle.n_zgrid; ++iz)
						{
							...
							for (ip = 0; ip < particle.n_pgrid; ++ip)
							{
								...
								Np0[ip] += (1. - 0.5 * alpha2_p.d2[ir][iz].s[ip]) * particle.cr_density.d2[ir][iz].s[ip];

								if (ip > 0)
									Np0[ip] += 0.5 * alpha1_p.d2[ir][iz].s[ip] * particle.cr_density.d2[ir][iz].s[ip - 1];

								if (ip < particle.n_pgrid - 1)
									Np0[ip] += 0.5 * alpha3_p.d2[ir][iz].s[ip] * particle.cr_density.d2[ir][iz].s[ip + 1];
							}
					...
					for (iz = 0; iz < particle.n_zgrid; ++iz)
					{
						...
						for (ip = 0; ip < particle.n_pgrid; ++ip)
						{
							...
							for (ir = 0; ir < particle.n_rgrid; ++ir)
							{
								...
								Nr0[ir] += (1. - 0.5 * alpha2_r.d2[ir][iz].s[ip]) * particle.cr_density.d2[ir][iz].s[ip];

								if (ir > 0)
									Nr0[ir] += 0.5 * alpha1_r.d2[ir][iz].s[ip] * particle.cr_density.d2[ir - 1][iz].s[ip];

								if (ir < particle.n_rgrid - 1)
									Nr0[ir] += 0.5 * alpha3_r.d2[ir][iz].s[ip] * particle.cr_density.d2[ir + 1][iz].s[ip];
							}
			...
			for (iy = 0; iy < particle.n_ygrid; ++iy)
			{
				...
				for (iz = 0; iz < particle.n_zgrid; ++iz)
				{
					...
					for (ip = 0; ip < particle.n_pgrid; ++ip)
					{
						...
						alpha1_x.d3[ix][iy][iz].s[ip] = particle.Dxx.d3[ix][iy][iz].s[ip] * pow(particle.dx, -2.);

						alpha1_y.d3[ix][iy][iz].s[ip] = particle.Dxx.d3[ix][iy][iz].s[ip] * pow(particle.dy, -2.);

						alpha1_z.d3[ix][iy][iz].s[ip] = particle.Dxx.d3[ix][iy][iz].s[ip] * pow(particle.dz, -2.);
						...
						for (ip = 1; ip < particle.n_pgrid - 1; ++ip)
						{
							...
							alpha1_p.d3[ix][iy][iz].s[ip] +=
								(-(particle.Dpp.d3[ix][iy][iz].s[ip] -
								 ...
								 2. * particle.Dpp.d3[ix][iy][iz].s[ip] / (particle.p[ip + 1] - particle.p[ip - 1]) + 2. * particle.Dpp.d3[ix][iy][iz].s[ip - 1] / particle.p[ip - 1]) /
								(particle.p[ip] - particle.p[ip - 1]);
							...
							alpha2_p.d3[ix][iy][iz].s[ip] +=
								...
								2. * particle.Dpp.d3[ix][iy][iz].s[ip] / (particle.p[ip + 1] - particle.p[ip - 1]) * (1. / (particle.p[ip + 1] - particle.p[ip]) + 1. / (particle.p[ip] - particle.p[ip - 1])) + 2. * particle.Dpp.d3[ix][iy][iz].s[ip] / (particle.p[ip] - particle.p[ip - 1]) / particle.p[ip];

							alpha3_p.d3[ix][iy][iz].s[ip] +=
								2. * particle.Dpp.d3[ix][iy][iz].s[ip] / (particle.p[ip + 1] - particle.p[ip - 1]) / (particle.p[ip + 1] - particle.p[ip]);
							// old Andy's method
							/*
		double p1=(particle.p[ip]+particle.p[ip-1])/2.;
		double p2=(particle.p[ip]+particle.p[ip+1])/2.;
			...
			for (ix = 0; ix < particle.n_xgrid; ++ix)
			{
				...
				for (iy = 0; iy < particle.n_ygrid; ++iy)
				{
					...
					for (iz = 0; iz < particle.n_zgrid; ++iz)
					{
						...
						for (ip = 1; ip < particle.n_pgrid - 1; ++ip)
						{ // bug fixed IMOS20020417

							alpha2_p.d3[ix][iy][iz].s[ip] +=
								particle.dpdt.d3[ix][iy][iz].s[ip] / (particle.p[ip + 1] - particle.p[ip]);

							alpha3_p.d3[ix][iy][iz].s[ip] +=
								particle.dpdt.d3[ix][iy][iz].s[ip + 1] / (particle.p[ip + 1] - particle.p[ip]);
...
#pragma omp parallel for schedule(dynamic) default(shared) private(iz, ix, iy)
			...
			strcmp(particle.name, "primary_TD_electron") == 0)
		{
			...
			if (flag == 0)
			{
				...
				//	sourceZ = dist * sin(b);
				//}
				if (galdef.random_coordinate == 0) // SB20190620
				{
					sourceX = galdef.SNR_x;
					sourceY = galdef.SNR_y;
					sourceZ = galdef.SNR_z;
					dist = galdef.distance;
				}
				...
				if (sourceX >= 0.0)
				{
					...
					particle.m_sourceX = galdef.SNR_x;												 // Saptashwa 2018/07
				}
				else
				{
					particle.m_sourceX = (double)(int)(sourceX * pow(10.0, 1) - 0.5) / pow(10.0, 1);
				}
				if (sourceY >= 0.0)
				{
					// double sourceYY = (double)(int)(sourceY * pow(10.0, 1) + 0.5) / pow(10.0, 1);
					//particle.m_sourceY = -sourceYY;
					particle.m_sourceY = galdef.SNR_y; // Saptashwa 2018/07
				}
				else
				{
					double sourceYY = (double)(int)(sourceY * pow(10.0, 1) - 0.5) / pow(10.0, 1);
				...
				if (sourceZ >= 0)
				{
					...
					particle.m_sourceZ = galdef.SNR_z; // Saptashwa 2018/07
				}
				else
				{
					particle.m_sourceZ = (double)(int)(sourceZ * pow(10.0, 1) - 0.5) / pow(10.0, 1);
			...
			if (flag2 == 0)
			{
				if (galdef.random_age == 0)
				{
					...
					// particle.m_source_age = (int)(galdef.source_age) * galdef.start_timestep; //SB 20191030
				}
	...
	*/
				...
				for (int irept = 1; irept <= nrept; ++irept)
				{
					...
					if (galdef.rm_nearbysource == 1)
					{
									...
									for (int iz = 0; iz < particle.n_zgrid; iz++)
										if (galdef.near_distance2 != 0 && galdef.near_distance3 != 0 && galdef.near_distance4 != 0)
										{
											spdistance = sqrt(particle.x[ix] * particle.x[ix] +
															  particle.y[iy] * particle.y[iy]); //+particle.z[iz]*particle.z[iz]); z not included in galactic plane
																								//cout<< "!!!!going good!!!!" ;
																								//cout<< spdistance ;
						...
						//cout<<"time done"<<particle.m_step_total_time<<endl;
					} //nearby source
		...
		for(iy=0; iy<particle.n_ygrid; ++iy){
	      ...
	      total_source_function=particle.primary_source_function+particle.secondary_source_function;
					...
					cout << "time done: " << particle.m_step_total_time << endl;
					cout << " total time - propagating time= " << particle.m_final_time - particle.m_step_total_time << endl;
					particle.m_step_total_time += dt / year2sec; //YO/20140611
