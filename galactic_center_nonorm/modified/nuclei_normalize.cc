...
// int Galprop::nuclei_normalize(Particle &particle)//SB20190731
{
   INFO("Entry");
   ostringstream buf;
   ...
   buf.str("");
   buf << "Galdef Source Normalization: ix x[ix] iy y[iy] iz z[iz] ip Ekin[ip] " << " " << galdef.source_normalization; //sb20191218
   // normalize all species except primary electrons since these are normalized independently
   for (int i = 0; i < n_species; i++)
   {
      if (strcmp(gcr[i].name, "primary_electrons") == 0)
         continue; // IMOS20050912

      //DM production should not be normalized BiXJ 2007/2/2
      if (strcmp(gcr[i].name, "primary_DM_positron") == 0)
         continue; // BiXJ 2007/2/2
      if (strcmp(gcr[i].name, "primary_DM_electron") == 0)
         continue; // BiXJ 2007/2/2
      if (strcmp(gcr[i].name, "primary_DM_antiproton") == 0)
         continue; // BiXJ 2007/2/2

      //time dependent injection should not be normalized YO/20140220 -> 20140408 changed
      if (strcmp(gcr[i].name, "primary_TD_positron") == 0)
         continue; // YO/20140220 -> 20140408 change

      // if (strcmp(gcr[i].name, "primary_TD_electron") == 0)
         // continue; // YO/20140408

      // don't re-normalize the DM annihilation products
      if (strcmp(gcr[i].name, "DM_positrons") == 0)
         continue; // IMOS20050912
      if (strcmp(gcr[i].name, "DM_electrons") == 0)
         continue; // IMOS20050912
      if (strcmp(gcr[i].name, "DM_antiprotons") == 0)
         continue; // IMOS20050912
      ...
      buf << "check normalization factor: " << gcr[i].normalization_factor; //sb20191218

      if (galdef.check_num == 1123)
         cerr << " >>>> BG nuclei normalization factor: " << gcr[i].normalization_factor << endl; //YO/20151222
